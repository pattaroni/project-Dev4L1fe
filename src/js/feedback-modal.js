...
function validateFeedback({ name, rating, descr }) {
  const errors = {};
  const n = (name ?? '').trim();
  if (n.length < 2 || n.length > 16) errors.name = 'Name must be 2–16 characters.';

  const r = Number(rating);
  if (!Number.isFinite(r) || r < 1 || r > 5) errors.rating = 'Rating must be between 1 and 5.';

  const d = (descr ?? '').trim();
  if (d.length < 10 || d.length > 512) errors.descr = 'Message must be 10–512 characters.';
...
function initFeedbackModal() {
  ...
  const showErrorUnder = (groupEl, msg) => {
    ...
    groupEl?.insertAdjacentHTML('beforeend', `<div class="form-error">${msg}</div>`);
  };
  ...
  form?.addEventListener('submit', async e => {
    ...
    const payload = {
      ...
      rating: Number((ratingHost?.querySelector('input[name="rating"]')?.value) || '0'),
    ...
    if (Object.keys(errors).length) {
      ...
      if (errors.rating) {
        ratingHost?.insertAdjacentHTML('afterend', `<div class="form-error">${errors.rating}</div>`);
      }
      return;
    }
    ...
    try {
      ...
      form.insertAdjacentHTML('afterbegin', `<div class="form-error">${msg}</div>`);
    } finally {
